...
- **Progress you can trust from anywhere** — the tab title counts up while the tab is hidden (`⏳ 41% · page 20 of 34 · Notes2A4`), the status line reads `page 20 of 34 · binarising 62% · ~40 s left` with a smoothed estimate that refreshes once a second, and every phase goes through one code path so the bar, the status and the title can never disagree
- **A background tab costs less, never more** — switching tabs skips the frame waits (the message-channel yield takes over), the live preview copy, the tab-title updates and the cosmetic thumbnails, and parses/paints nothing; the strip of previews parks itself with a note and finishes the moment the tab is back. Measured in the smoke suite: a hidden 220 dpi run finishes in the same time as a visible one, with no frame ever requested
- **The wait says what it is** — the status line reads "page 12 of 34 · binarising 62% · ~40 s left" (an estimate from the work already done), the live panel notes that the small tiles are quick thumbnails while it shows the full-resolution page, and the preview strip pauses while a run is going (no CPU fight) and finishes itself once the result is on screen
