...
- **The wait says what it is** — the status line reads "page 12 of 34 · binarising 62%" and a **"~40 s left" chip** sits right next to the bar (an estimate extrapolated from the work already done; it says "estimating time left…" until there is enough to go on, and disappears the moment the run is done), the live panel notes that the small tiles are quick thumbnails while it shows the full-resolution page, and the preview strip pauses while a run is going (no CPU fight) and finishes itself once the result is on screen
