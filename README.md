...
- **The heavy pixels run in a worker** — `worker-raster.js` runs the print-saver map over the rendered page and the PNG/JPEG encode off the main thread; strips are transferred (never copied) and the finished page comes back as encoded bytes. The maths is not re-implemented: the worker imports `converter.js` and uses the very same `printSaver.pieces`, so a worker-built page is byte-identical (verified by decoding the returned PNG and comparing it with the main-thread map). If a browser cannot start a worker, the apps fall back to the main-thread banded pipeline without breaking a single conversion
- **Pages are rendered in strips** — a 220 dpi A4 page is millions of pixels of canvas; instead of asking pdf.js for that in one call (which blocks for seconds and triggers Chrome's "Page Unresponsive" dialog), each page is drawn as ~96-row strips with whole-pixel `offsetY` offsets, fed straight into the banded maps. Same pixels, no giant canvas, no long block
- **Crisp HD output (Invert Lab)** — pages render direct at the target dpi with no supersample-and-smooth step (that melted 0.5 pt strokes into grey blur); PNG + 220 dpi are the defaults, so zoomed-in lines stay razor-sharp — best results win over small files
...
- **Exact vector Pure B&W** (Invert Lab) — every paint re-thresholded to solid black or paper white inside the PDF's own content streams (same luma rule as raster pure): text stays text, 0.5 pt hairlines stay razor-sharp at 500% zoom, light pages pass through untouched, images/patterns pass through — no rasterising, so Sharpness / Encoding do not apply
...
**`test/convert.test.mjs` — 118 assertions** on the shipped converter: 2-up geometry vs. the reference demo (±3 pt); 4-up pair-column geometry vs. the measured landscape demo cells (±6 pt per corner); band/flush/gutter invariants; shrink-to-fit; partial sheets; blank-page robustness; 400 → 200 and 400 → 100; sheet-numbering (7 positions × 5 styles × start-at × 3 sizes); print-saver pixel map (black→white, white→black, colours→black), auto dark-page detection, HQ coverage map (quarter/half-ink blocks → grey ramp, chroma rule, light-page passthrough), pure B&W hard threshold (0 grey pixels on a mixed page), 4-up dotted separator (vertical seam geometry, margin inset, dash op present in the PDF and absent when off or in 2-up, shipped UI is the vertical toggle only), and raster-pack layout in both 2-up and 4-up; vector Pure B&W (colour-operator remap to solid 0/1 incl. CMYK + SC/SCN + q/Q + pattern/inline-image passthrough, text and hairline widths intact, light-page byte-identical passthrough, still pure vector); end-to-end builds on the in-repo sample notes PDFs (`test/fixtures/`, so the suite needs no files outside the repo).

**`test/app-smoke.test.mjs` — 82 checks**: the real `app.js` / `app4up.js` / `app-invert.js` are loaded into a stubbed DOM and actually run — a fixture PDF is dropped in, Start is clicked and every mode is exercised (vector pack, Print-Saver in all colour styles, 4-up with the dotted separator, Invert Lab in black-ink, raster-negative and exact-vector modes). Shipped builds have twice failed at run time in exactly this place (a missing helper name), which grep-based checks cannot see.
...
**`test/overlay.test.mjs` — 67 checks**: the standalone `overlays.js` engine — option normalisation (junk → safe defaults), all five number styles with start-at totals, six in-bounds number positions, ruling/separator geometry, a real pdf-lib draw (line ops, dash op, round caps, decoded number label in the content stream), all 10 line styles, packer white-band detection (clean/noisy gaps, vertical-axis symmetry, dotted-separator merge, blank/dark/text-page rejection, sub-12pt rejection, flank-content + nearest-centre selection), real-geometry regression proofs (the actual `quadLayout`/`sheetLayout` bands are covered in auto and fixed-8mm gap modes, partial sheets rejected, fixed-gap padding never wins), the Difference unflip restore (single + union cross-split geometry + real-page stream), white-cross detection on both axes, the ink-mode band restore to paper white (`whitenBand`), the canvas live-preview twin (`drawPreview`, count-identical with the PDF on all 10 styles), band-confined ruling, `bandOnly` composition, and degenerate-input safety.
**`test/band-e2e.test.mjs` — 21 checks**: the true end-to-end proof — builds a real 4-up (landscape) + 2-up (portrait) PDF with `converter.buildFromImages`, extracts the embedded slide images back out of the PDF bytes, composites each page at the app's 0.35 detection scale, proves the real `findBand` covers the real layout band, then flips with the real `invertPixels` lifted from the shipped `app-invert.js` and proves band rows stay pixel-identical white while slide rows flip — plus the union safety locks (no vertical band on our own output), the white-cross union-masking proof, and the ink-mode proofs (the unmasked band demonstrably turns to solid ink on a dark page; with the restore it stays 255 white in black-ink, pure b&w and white-paper modes while slides become paper).
