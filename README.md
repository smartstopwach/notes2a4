...
- **Dotted separators** (4-up Studio): a small toggle draws a black dotted cut-line down the middle of the sheet — **vertical**, **horizontal** or **both (cross)** — so the four panels read as separate notes on one printed page. Drawn as a real dashed PDF line with round caps (`[0.9 3.2] 0 d`, 1.1 pt) through the column seam and the row boundary, inset to your margin; the live preview and the shipped PDF come from the one shared helper (`sepLines`), and 2-up output is never touched
...
**`test/convert.test.mjs` — 90 assertions** on the shipped converter: 2-up geometry vs. the reference demo (±3 pt); 4-up pair-column geometry vs. the measured landscape demo cells (±6 pt per corner); band/flush/gutter invariants; shrink-to-fit; partial sheets; blank-page robustness; 400 → 200 and 400 → 100; sheet-numbering (7 positions × 5 styles × start-at × 3 sizes); print-saver pixel map (black→white, white→black, colours→black), auto dark-page detection, HQ coverage map (quarter/half-ink blocks → grey ramp, chroma rule, light-page passthrough), pure B&W hard threshold (0 grey pixels on a mixed page), 4-up dotted separators (seam/boundary geometry, margin inset, mixed row ratios, dash op present in the PDF and absent when off or in 2-up), and raster-pack layout in both 2-up and 4-up; end-to-end builds on the in-repo sample notes PDFs (`test/fixtures/`, so the suite needs no files outside the repo).
