...
**`test/overlay.test.mjs` — 27 checks**: the standalone `overlays.js` engine — option normalisation (junk → safe defaults), all five number styles with start-at totals, six in-bounds number positions, ruling/separator geometry, a real pdf-lib draw (line ops, dash op, round caps, decoded number label in the content stream), all 10 line styles, and degenerate-input safety.
